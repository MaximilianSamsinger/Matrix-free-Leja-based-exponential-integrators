...
\begin{document}
	...
	\begin{align}\label{eq:LinearAdvDif}
	\begin{split}
...
%Even though $A$ can be represented as a sparse matrix, it is more memory-efficient to treat it as a matrix-free linear operator.
	
	
	\subsection{Power iteration analysis}\label{poweritanalysis}
	\begin{figure}[t]
	...
	%,\quad j=1,\dots, N.
	%\]
We investigate the rate of convergence of the power method to the absolutely largest eigenvalue $\lambda_{max}$ of $A$.
...
\begin{align*}
\frac{h^{4n}}{N}\norm{A^nv}_2^2 
&= \frac{h^{4n}}{N}\sum_{j=0}^{N-1}{\abs{\lambda_j}^{2n}}
= \frac{1}{N}\sum_{j=0}^{N-1}{\abs{h^2\lambda_j}^{2n}} \\
&= \frac{1}{N}\sum_{j=0}^{N-1} \left(\left(4a-2bh\right)^2 \sin^4\left(\frac{\pi j}{N}\right) + bh\sin^2\left(\frac{2\pi j}{N-1}\right)\right)^n \\
...
\end{align*}
where $\operatorname{B}$ is the beta function. The first equality holds since all eigenvectors are orthogonal. When the limit is taken all terms depending on $h$ vanish. The remaining sum is a left Riemann sum of the integrable function $x\to (4a)^{2n}\sin^{4n}(\pi x)$. 
In the limit we underestimate the absolutely largest eigenvalue $\lambda_{max}$ after $n\ge2$ power iterations by a factor of $\alpha_n$ \ref{table:alphan}, where $\alpha_n$ is given by

\begin{align*}
\lim\limits_{N\to\infty} \frac{\norm{A^{n-1}v}_2}{\norm{A^nv}_2}\abs{\lambda_{max}} &= 
\lim\limits_{N\to\infty}\frac{\frac{h^{2n-2}}{\sqrt{N}}\norm{A^{n-1}v}_2}{\frac{h^{2n}}{\sqrt{N}}\norm{A^nv}_2}\abs{h^2\lambda_{max}} \\
&= \sqrt{
	\frac{(4a)^{2n-2} \operatorname{B}(2n-1.5,0.5)}
		{(4a)^{2n} \operatorname{B}(2n+0.5,0.5)}}4a\\
&= \sqrt{\frac{\operatorname{B}(2n-1.5,0.5)}{\operatorname{B}(2n+0.5,0.5)}}.
\end{align*}
\begin{table}
	\centering
...
We have
\begin{align*}
\frac{\operatorname{B}(2n-1.5,0.5)}{\operatorname{B}(2n+0.5,0.5)} &= 
\frac{\Gamma(2n + 1)\Gamma(2n - 1.5)}{\Gamma(2n - 1)\Gamma(2n + 0.5)} \\&= 
\frac{(2n)!}{(2n-2)!}
	\frac{\Gamma(2n - 1.5)}{\Gamma(2n + 0.5)}
	\frac{\Gamma(2n)}{\Gamma(2n)}
	\frac{\Gamma(2n-2)}{\Gamma(2n-2)} \\&=
2n(2n-1) 
	\frac{2^{5-4n}\sqrt\pi}{2^{1-4n}\sqrt\pi}
	\frac{\Gamma(4n-4)}{\Gamma(4n)}
	\frac{\Gamma(2n)}{\Gamma(2n-2)} \\&=
32n(2n-1)
	\frac{(4n-5)!}{(4n-1)!}
	\frac{(2n-2)!}{(2n-4)!} \\&=
\frac{32n(2n-1)(2n-2)(2n-3)}{(4n-1)(4n-2)(4n-3)(4n-4)}\\&=
\frac{4n(4n-6)}{(4n-1)(4n-3)}\\&=
\left(1+\frac{1}{4n-1}\right)\left(1-\frac{3}{4n-3}\right)
\end{align*}
For the third equality we applied the duplication formula for the gamma function. All in all we underestimate the absolutely largest eigenvalue $\lambda_{max}$ by a factor of 
\[
\alpha_n = 
\lim_{N\to\infty}\frac{\norm{A^{n-1}v}}{\norm{A^{n}v}}\abs{\lambda_{max}} =
\sqrt{\left(1+\frac{1}{4n-1}\right)\left(1-\frac{3}{4n-3}\right)}
\]
at the limit $N\to\infty$. 
\subsection{Power iteration experiment}
As discussed in section \ref{matrixfreeimplementation} we need to estimate the absolutely largest eigenvalue $\lambda_{max}$ of $A$ for the matrix-free \texttt{expleja} algorihm. However, we can only guarantee convergence if we overestimate $\lambda_{max}$. Therefore we have to include a safety-factor by which we multiply the output of the power method.
...
\begin{figure}[h!]
	...
	\caption{Space dimension $N$ vs costs \texttt{mv} per timestep $s$ for matrix-free single precision \texttt{expleja}. Results are only shown if they achieve \precision\ precision.} \label{fig:Poweriterations}
\end{figure}
A priori it is unclear whether this result holds true for arbitrary $v$ and $N$. Therefore we do not know whether or not if the matrix-free Leja method converges. We solve the initial value problem \label{eq:LinearAdvDif} for a fixed number of substeps $s$ with fixed interpolation degree $m=100$ per substep while still allowing for an early termination of the interpolation if the error is small enough, see Figure \ref{fig:Poweriterations}. While the algorithm did not converge for all combinations of $N$, $\text{sf}$ and P\'eclet numbers we tried, we observed that $n=4$ and $\text{sf}=1.1$ is a robust choice independent of $N$ for our initial vector $u_0$. This combination also happened to be optimal for the power method analysis in the previous section, since $\frac{1}{\alpha_4}\approx 1.1$. \\
For all future experiments we choose $\text{sf}=1.1$ and $n=4$ for the matrix-free \texttt{expleja} algorithm. As an extra optimization, we save the approximated eigenvector for $\lambda_{max}$ when integrating over multiple timesteps. We use it as the initial vector for the next time step. Furthermore we terminate the power method early if the relative change of the approximated eigenvalue is smaller than $0.01$. 
...
%	at the limit $N\to\infty$. 
	...
	\partial_tu &= Au.
	\end{align*} 
	Some eigenvalues of $A$ can have an extremely large negative real part. Therefore, since no explicit Runge-Kutta method is A-stable, this imposes very stingend conditions on the time step size $\tau$ for $\operatorname{rk2}$ and $\operatorname{rk4}$.\footnote{See section \ref{CFL}} We will refer to the Courant-Friedrich-Lewy (CFL) conditions imposed by the advective and diffusive part of $A$ respectively by $C_{adv}$ and $C_{dif}$.  
