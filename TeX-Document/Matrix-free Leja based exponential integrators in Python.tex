...
\begin{document}
	...
	\end{figure} 
\noindent The solution of the differential equation is given by $e^{0.1A}u_0$, which can be approximated using the Leja method \ref{table:Experiment1}. For the matrix-free case we need to compute the spectral radius of $A$, which can be done using the power method.
%Even though $A$ can be represented as a sparse matrix, it is more memory-efficient to treat it as a matrix-free linear operator.
	...
	%,\quad j=1,\dots, N.
	%\]
In this section we investigate the rate of convergence of the power method to the absolutely largest eigenvalue $\lambda_{max}$ of $A$.
...
In the limit we underestimate the absolutely largest eigenvalue $\lambda_{max}$ after $n\ge2$ power iterations by a factor of

\begin{align*}
\lim\limits_{N\to\infty} \frac{\norm{A^{n-1}v}_2}{\norm{A^nv}_2}\abs{\lambda_{max}} &= 
\lim\limits_{N\to\infty}\frac{\frac{h^{2n-2}}{\sqrt{N}}\norm{A^{n-1}v}_2}{\frac{h^{2n}}{\sqrt{N}}\norm{A^nv}_2}\abs{h^2\lambda_N} \\
&= \sqrt{
