...
%				+ \beta\ \vec{1}\!\cdot\!\nabla u^2 + u(u-0.5).
	...
	\begin{figure}[H]
		...
		\caption{Comparison of different integrators for the one-dimensional case and the absolute tolerances \textit{half} ($\operatorname{tol} = 2^{-10}$) and single \textit{single} ($\operatorname{tol} = 2^{-24}$). Shows the }
	\end{figure}
	
	
	\subsection{One-dimensional advection-diffusion equation} \label{sec:Experiment1dim}
