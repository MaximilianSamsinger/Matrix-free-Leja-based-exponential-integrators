...
%	
	...
	\begin{align*}
	\partial_tu &= F(u) = \alpha\partial_x(u\partial_xu) + \beta\partial_x(u^2) + u(u-0.5)
	\end{align*}
	...
	\[
		\partial_{u}F(u) = 
			\alpha(\partial_{xx} + \partial_{x}u\operatorname{Id} + u\partial_{x}) 
	...
	All instances of $\partial_{xx}$ are discretized with centred finite differences. We approximate $\partial_{x}$ using the upwind scheme for numerical stability. The matrix representation of both discretized operators is given by
	\[
		INSERT MATRICES
	\]
	
	\[
		F(u) = (\alpha)\partial_{xx} + 
	\]
	\[
	\]
	
	After discretization the
	\[ 
