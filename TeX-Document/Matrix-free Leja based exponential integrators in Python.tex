...
%	
	...
	The action of the matrix exponential is approximated with the matrix-free \texttt{expleja} algorithm. At each time step we compute the optimal interpolation degree and substep parameter $s$ according to \eqref{eq:msmatrixfree}. We choose $\theta_m$ based on \texttt{tol} as showin in Table~\ref{table:thetam}. During the Newton interpolation we calculate the 2-norm of each term added to the polynomial. We use the two most recent terms as an error estimate. We terminate the Newton interpolation early if the relative error is smaller than $\texttt{tol}/N_\tau$. 
...
\end{thebibliography}
\newcommand{\plotwidth}{0.97866\columnwidth}\filbreak
