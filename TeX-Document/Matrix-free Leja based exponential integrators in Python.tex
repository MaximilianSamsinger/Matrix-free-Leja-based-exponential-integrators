...
\subsection{Results} \label{sec:Results}
	...
	These figures show the matrix-free case. Since \texttt{exprb3} was always outperformed by either \texttt{exprb2} or \texttt{exprb4} we exclude it from all plots to increase readability. 
	The results for CSR-matrix case, as well as all other choices for $\alpha$ and $\beta$, are shown in Appendix~\ref{sec:1dim} and \ref{sec:2dim}. We discuss the results of the matrix-free case, unless otherwise noted.  
...
Finally, we investigate the role of substepping for \texttt{expleja}. We recompute the original experiments and considered the one-dimensional, matrix-free case. We fixed the interpolation degree~$m=100$ and disabled substepping by selecting~$s=1$. Early termination for the Newton interpolation is still enabled. The results are shown in Figures~\ref{fig:1DNonlinearHighPeSubsteppingDisabled}, \ref{fig:1DNonlinearLowPeSubsteppingDisabled} and Appendix \ref{sec:1dim}. Substepping is overall beneficial to reduce the costs of \texttt{exprb2} and \texttt{exprb4}.
...
\subsection*{One-dimensional case, high diffusion, low advection, substepping disabled}
\begin{figure}[H]
	\newcommand{\dif}{\detokenize{α}=0.1}
	\newcommand{\adv}{\detokenize{β}=0.01}
	\centering
	\includegraphics[width=\columnwidth]{../Figures/Experiment2 - no substepping/multi, \dif, \adv.pdf}
	\caption{One-dimensional matrix-free case. Comparison of different matrix-free integrators given relative tolerances \textit{half} ($\operatorname{tol} = 2^{-10}$) and single \textit{single} ($\operatorname{tol} = 2^{-24}$). Substepping is disabled for \texttt{expleja}. We calculated the cost-minimizing time step size for each integrator, as shown in the second row. The top row displays the corresponding cost incurred. The bottom row shows the average number of Jacobian-vector products computed per time step. We chose $\alpha\dif$, $\beta\adv$. The dotted line indicates the largest time step size, where the CFL conditions are not violated.}
	\label{fig:1DNonlinearHighPeSubsteppingDisabled}
\end{figure}

\subsection*{One-dimensional case, low diffusion, high advection, substepping disabled}
\begin{figure}[H]
	\newcommand{\dif}{\detokenize{α}=0.01}
	\newcommand{\adv}{\detokenize{β}=1}
	\centering
	\includegraphics[width=\columnwidth]{../Figures/Experiment2/multi, \dif, \adv.pdf}
	\caption{One-dimensional matrix-free case. Comparison of different matrix-free integrators given relative tolerances \textit{half} ($\operatorname{tol} = 2^{-10}$) and single \textit{single} ($\operatorname{tol} = 2^{-24}$). Substepping is disabled for \texttt{expleja}. We calculated the cost-minimizing time step size for each integrator, as shown in the second row. The top row displays the corresponding cost incurred. The bottom row shows the average number of Jacobian-vector products computed per time step. We chose $\alpha\dif$, $\beta\adv$. The dotted line indicates the largest time step size, where the CFL conditions are not violated.}
	\label{fig:1DNonlinearLowPeSubsteppingDisabled}
\end{figure}
\clearpage
...
\begin{thebibliography}{13}
	...
	\bibitem{lejarev} M. Caliari, P. Kandolf, A. Ostermann, S. Rainer. The Leja method revisited: Backward error analysis for the matrix exponential, SIAM Journal on Scientific Computation 38.3 (2016), A1639-A1661.
	...
	\bibitem{python} Python Software Foundation. Python Language Reference, version 3.7. Available at https://www.python.org. Manual at https://docs.python.org/3.7/. [Online; accessed 2020-11-01]
	...
	\bibitem{scipy} P. Virtanen et al. SciPy 1.0: Fundamental Algorithms for Scientific Computing in Python. Nature methods 17.3 (2020) 261-272.
...
\foreach \DIF in {0.1}
\foreach \ADV in {1, 1, 0.1}
{
	\begin{figure}[H]
		\centering
		\includegraphics*[width=\plotwidth]{../Figures/Experiment2/multi, \detokenize{α}=\DIF, \detokenize{β}=\ADV.pdf}
		\caption{One-dimensional matrix-free case of the experiment specified in Section~\ref{sec:Experiments}. Comparison of different matrix-free integrators given relative tolerances \textit{half} ($\operatorname{tol} = 2^{-10}$) and \textit{single} ($\operatorname{tol} = 2^{-24}$). We calculated the cost-minimizing time step size for each integrator, as shown in the second row. The top row displays the corresponding cost incurred. The bottom row shows the average number of Jacobian-vector products computed per time step. We chose ${\alpha=\DIF}$, ${\beta=\ADV}$. The dotted line indicates the largest time step size, where the CFL conditions are not violated.}
	\end{figure}
}
\foreach \DIF in {0.01}
...
\subsection*{Matrix-free case, substepping disabled}
...
\begin{figure}[H]
	\centering
	\includegraphics*[width=\plotwidth]{../Figures/Experiment2 - no substepping/multi, \detokenize{α}=\DIF, \detokenize{β}=\ADV.pdf}
	\caption{One-dimensional matrix-free case of the experiment specified in Section~\ref{sec:Experiments}. Comparison of different matrix-free integrators given relative tolerances \textit{half} ($\operatorname{tol} = 2^{-10}$) and \textit{single} ($\operatorname{tol} = 2^{-24}$). Substepping is disabled for \texttt{expleja}. We calculated the cost-minimizing time step size for each integrator, as shown in the second row. The top row displays the corresponding cost incurred. The bottom row shows the average number of Jacobian-vector products computed per time step. We chose ${\alpha=\DIF}$, ${\beta=\ADV}$. The dotted line indicates the largest time step size, where the CFL conditions are not violated.}
\end{figure}
}
\foreach \DIF in {0.01}
\foreach \ADV in {0.1, 0.01}
...
\foreach \ADV in {1, 0.1, 0.01}
{
	\begin{figure}[H]
		...
		\caption{One-dimensional CSR-matrix case of the experiment specified in Section~\ref{sec:Experiments}. Comparison of different matrix-free integrators given relative tolerances \textit{half} ($\operatorname{tol} = 2^{-10}$) and \textit{single} ($\operatorname{tol} = 2^{-24}$). We calculated the cost-minimizing time step size for each integrator, as shown in the second row. The top row displays the corresponding cost incurred. The bottom row shows the average number of Jacobian-vector products computed per time step. We chose ${\alpha=\DIF}$, ${\beta=\ADV}$. The dotted line indicates the largest time step size, where the CFL conditions are not violated.}
	\end{figure}
}
...
\foreach \ADV in {1, 0.1, 0.01}
{
	\begin{figure}[H]
		...
		\caption{Two-dimensional CSR-matrix case of the experiment specified in Section~\ref{sec:Experiments}. Comparison of different matrix-free integrators given relative tolerances \textit{half} ($\operatorname{tol} = 2^{-10}$) and \textit{single} ($\operatorname{tol} = 2^{-24}$). We calculated the cost-minimizing time step size for each integrator, as shown in the second row. The top row displays the corresponding cost incurred. The bottom row shows the average number of Jacobian-vector products computed per time step. We chose ${\alpha=\DIF}$, ${\beta=\ADV}$. The dotted line indicates the largest time step size, where the CFL conditions are not violated.}
	\end{figure}
}
