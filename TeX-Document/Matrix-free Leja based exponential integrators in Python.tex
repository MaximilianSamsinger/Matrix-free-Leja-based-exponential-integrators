...
In order to keep the solution from vanishing, we only consider coefficients $a$, $b\in[0,1]$ such that $a + b = 1$. We will denote the advection-diffusion ratio scaled by grid size $\Delta x = {(N - 1)}^{-1}$ by
\[\operatorname{Pe} = \frac{b \Delta x}{2a}\].
